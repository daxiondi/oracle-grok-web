...
| `-m, --model <name>`              | Single model. See [Mythical Pro Agents](mythical-pro-agents.md); `manus` selects the Manus web provider. |
...
`--model manus` is browser-only and requires a signed-in Chrome session. See [Manus Web Support](manus.md).
